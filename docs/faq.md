...
| What does Skip display-only DSP when hidden do? | It is on by default. When graphs cannot be shown—while minimized, in Mini Player, or on a hidden browser page—EffeTune bypasses Analyzer effects to reduce processing load. Audio passes through them unchanged, and analysis resumes when graphs can be shown. Turn it off if analysis must continue in the background. |
