...
| Que fait « Ignorer le DSP d’affichage lorsque l’application est masquée » ? | Cette option est activée par défaut. Lorsque les graphiques ne peuvent pas être affichés, EffeTune contourne les effets Analyzer afin de réduire la charge de traitement. C’est le cas lorsque l’Effect Pipeline n’est pas affiché, par exemple dans la Bibliothèque musicale, dans l’onglet Lecteur sur mobile ou pendant un Double Blind Test, ainsi que quand EffeTune est réduit, en mini-lecteur ou lorsque la page du navigateur est masquée. Le son traverse ces effets sans modification et l’analyse reprend dès que les graphiques peuvent être affichés. Désactivez cette option si l’analyse doit continuer en arrière-plan. |
