...
Обрабатывайте до четырёх вкладок Chrome или Edge с отдельными цепочками эффектов, выбором пресета по URL и настройкой частоты дискретизации без виртуального аудиоустройства. См. [руководство по расширению браузера](browser-extension.md).
...
- Настраиваемые композиции Visualizer с графиками, обложкой и сведениями о треке
...
- Нажмите кнопку скорости рядом с Shuffle, чтобы открыть всплывающее окно. Выберите один из девяти вариантов или задайте скорость от 0,25x до 4x с шагом 0,01x с помощью горизонтального ползунка или числового поля. Высота тона сохраняется.
...
### Просмотр звука в Visualizer

Откройте **Visualizer** в верхней части окна, на мобильной вкладке **Плеер** или через меню настольного приложения **Вид**. Выберите встроенную композицию или расположите графики, обложку и сведения о треке с помощью **Edit**. Изменения видны сразу и возвращаются при следующем запуске; именованную копию сохраните в окне пресетов. Подробнее — в [руководстве Visualizer](visualizer.md).
...
Откройте **Настройки > Резервная копия / Восстановление**, чтобы перенести сохранённые пресеты цепочки, пресеты отдельных эффектов, импульсные характеристики и измерения между веб-приложением, настольным приложением и расширением браузера. Можно выбирать отдельные элементы или целые категории. При выборе пресета автоматически выбираются необходимые ему данные. Если снять выбор с таких данных, зависимые от них пресеты также будут исключены из выбора.

Веб- и настольная версии также включают в резервную копию именованные пресеты Visualizer и их фоновые изображения, но не текущую рабочую композицию.

Параметры **Включить данные измерений** и **Включить импульсные характеристики** включены по умолчанию, поэтому для восстановления достаточно самого файла резервной копии. Если отключить один из них, для этого типа данных сохранятся только ссылки. Восстановить их можно лишь при наличии соответствующих данных в месте назначения. Импульсные характеристики восстанавливаются в **Impulse Response Library**, которой управляет EffeTune. Если данные с таким же содержимым уже есть, используются они; если совпадает только имя, новый элемент добавляется с номером в имени. Текущая цепочка, громкость, выбранный пресет, настройки устройств и правила URL расширения не меняются.

Размер каждого файла `.effetune_backup` ограничен 256 MB. Если выбранные данные превышают лимит, разделите их на несколько резервных копий. Если восстановление прервётся, уже восстановленные элементы останутся доступны, и вы сможете безопасно повторить попытку с тем же файлом. Перед передачей копии другим людям проверьте выбранные данные: файл может содержать имена и комментарии пресетов, исходные файлы импульсных характеристик и сведения об измерениях. Музыкальные файлы, данные Music Library, настройки приложения, выбранные устройства, правила URL, учётные данные и несохранённая текущая цепочка в копию не входят. Пресет можно перенести в расширение браузера, сохранить и снова включить в резервную копию, даже если его маршрутизация или эффекты не поддерживаются расширением.
