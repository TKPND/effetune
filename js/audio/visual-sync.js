...
const rules = {
    ...
    NoteSpectrogramPlugin: rule((p, rate) =>
        clamp(Math.round(rate * 16384 / 48000), 32, 65536) / 2 +
...
export function audiblePerformanceTime({ endFrame, generationFrames = 0, tapFrames = 0,
    outputDelayFrames = 0, sampleRate, outputTimestamp, fallback }) {
    const timestamp = outputTimestamp && Number.isFinite(outputTimestamp.contextTime) &&
        Number.isFinite(outputTimestamp.performanceTime) && outputTimestamp.performanceTime > 0
        ? outputTimestamp : {
            contextTime: fallback.currentTime - (fallback.outputLatency || fallback.baseLatency || 0),
            performanceTime: fallback.performanceTime
        };
    return timestamp.performanceTime +
        ((endFrame - generationFrames + tapFrames + outputDelayFrames) / sampleRate - timestamp.contextTime) * 1000;
}
