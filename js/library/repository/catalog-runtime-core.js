...
export const catalogState = {
  ...
  runtimeEventSink: () => {},
  storageEstimate: { quota: 0, usage: 0 },
  contextTtlMs: undefined,
  maxContexts: undefined,
  contextWalCapBytes: undefined
};
...
export function bindCatalogPlatform(platform) {
  ({
    Buffer,
    ENTITY_DEFINITIONS,
    artworkClaimMatchesCurrentTrack,
...
function removeLegacyPlaybackOperations() {
  runDurableTransaction(() => {
    const operationIds = catalogState.database.prepare(`
    ...
    const deleteProgress = catalogState.database.prepare('DELETE FROM operation_progress WHERE operation_id = ?');
    const deleteSavePages = catalogState.database.prepare('DELETE FROM sequence_save_pages WHERE operation_id = ?');
    const deleteOperation = catalogState.database.prepare('DELETE FROM operation_jobs WHERE operation_id = ?');
...
function upsertFolders(payload) {
  ...
  const statement = catalogState.database.prepare(`
    INSERT INTO folders(
...
function upsertTracks(payload) {
  ...
  const prepareUpsert = rowCount => catalogState.database.prepare(`
    INSERT INTO tracks(
  ...
  const deleteFts = catalogState.database.prepare(`
...
function normalizeTrack(track, index) {
  ...
  return {
    ...
    searchText: catalogState.modules.searchNormalizer.createCompactSearchText(
...
function normalizeContextQuery(payload) {
  ...
  const tokens = catalogState.modules.searchNormalizer.tokenizeSearchQuery(rawQuery);
  ...
  const sortDefinitions = entityType === 'track' ? catalogState.modules.orderContract.TRACK_ORDER_SPECS : definition.sorts;
...
function readContextPage(payload) {
  ...
  if (payload.cursor !== undefined && payload.cursor !== null) {
    const envelope = catalogState.modules.cursorCodec.decodeCursor(payload.cursor, {
      endpoint: 'tracks',
...
function resolveEntityAnchor(payload) {
  ...
  const prefix = payload.prefix == null ? null : catalogState.modules.searchNormalizer.normalizeSearchText(
...
function resolveNamedEntityContextOrdinal(context, { entityId, prefix, mode }) {
  ...
  if (mode === 'prefix') {
    ...
    const prefixKey = Buffer.from(catalogState.modules.orderContract.encodeCanonicalSearchKey(prefix), 'hex');
...
function createPlaylistOrder() {
  const descriptor = catalogState.modules.canonicalOrder.createCanonicalOrderDescriptor({
    id: `tracks.playlist-position.asc.${catalogState.modules.orderContract.MUSIC_LIBRARY_ORDER_VERSION}`,
    endpoint: 'tracks',
...
function createEntityOrder(context, definition) {
  const fieldSpecs = context.sort === 'name'
    ? catalogState.modules.orderContract.ENTITY_NAME_ORDER_SPECS[context.entityType]
  ...
  const descriptor = catalogState.modules.canonicalOrder.createCanonicalOrderDescriptor({
    id: `entities.${context.entityType}.${context.sort}.${context.direction}.${catalogState.modules.orderContract.MUSIC_LIBRARY_ORDER_VERSION}`,
    endpoint: context.endpoint,
...
function encodeEntityBoundaryCursor(context, order, row, continuation) {
  return catalogState.modules.cursorCodec.encodeCursor({
    queryFingerprint: context.queryFingerprint,
    snapshotVersion: context.snapshotVersion,
    sortSpecId: order.descriptor.id,
    continuation,
...
function ensureDirectoriesSynchronized() {
  const generation = Number(catalogState.database.prepare(
    'SELECT generation FROM directories_sync WHERE id = 1'
  ).get()?.generation ?? 0);
  const watermark = Number(catalogState.database.prepare(
    "SELECT value FROM meta WHERE key = 'directories_watermark'"
  ).get()?.value ?? -1);
  if (watermark === generation) return;
  runDurableTransaction(() => {
    catalogState.database.prepare('DELETE FROM directories').run();
...
function createOrder(sort, direction) {
  const fields = catalogState.modules.orderContract.TRACK_ORDER_SPECS[sort].map(field => ({
    ...field,
    column: trackOrderColumn(field.field),
    direction
  }));
  const descriptor = catalogState.modules.canonicalOrder.createCanonicalOrderDescriptor({
    id: `tracks.${sort}.${direction}.${catalogState.modules.orderContract.MUSIC_LIBRARY_ORDER_VERSION}`,
    endpoint: 'tracks',
...
function getOperationStatus(payload) {
  ...
  `).get(operationId);
  if (!row) return null;
  const progress = catalogState.database.prepare(`
...
function recordOperationProgress(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const current = catalogState.database.prepare(`
...
function releaseOperationSnapshots(operationId) {
  const owned = catalogState.database.prepare(`
  ...
  `);
  const removeOwner = catalogState.database.prepare(`
    DELETE FROM snapshot_object_owners
    WHERE snapshot_id = ? AND owner_kind = 'operation' AND owner_id = ?
  `);
  const markCollectable = catalogState.database.prepare(`
...
function gcTerminalOperations(payload) {
  ...
  return runDurableTransaction(() => {
    const candidates = catalogState.database.prepare(`
...
function appendOperationSnapshotItems(payload) {
  ...
  return runDurableTransaction(() => {
    const snapshot = catalogState.database.prepare(`
    ...
    const tail = catalogState.database.prepare(`
      SELECT COALESCE(MAX(ordinal), -1) AS ordinal FROM snapshot_items WHERE snapshot_id = ?
    `).get(snapshotId);
    const insert = catalogState.database.prepare(`
...
function sealOperationSnapshot(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    if (!snapshot) throw createCatalogError('snapshotNotFound', 'Snapshot does not exist');
    if (snapshot.state !== 'staging') throw createCatalogError('snapshotSealed', 'Snapshot is already sealed');
    const count = Number(catalogState.database.prepare(`
...
function queryOperationSnapshot(payload) {
  ...
  `).get(snapshotId);
  if (!snapshot) throw createCatalogError('snapshotNotFound', 'Snapshot does not exist');
  const items = catalogState.database.prepare(`
...
function gcOperationSnapshots(payload) {
  ...
  return runDurableTransaction(() => {
    const items = catalogState.database.prepare(`
      ...
      LIMIT ?
    `).all(limit);
    const deleteItem = catalogState.database.prepare(`
    ...
    if (remaining > 0) {
      const snapshots = catalogState.database.prepare(`
        ...
        LIMIT ?
      `).all(remaining);
      const remove = catalogState.database.prepare(`
...
function appendPlaybackSequenceItems(payload) {
  ...
  return runDurableTransaction(() => {
    const sequence = catalogState.database.prepare(`
...
function sealPlaybackSequence(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const aggregate = catalogState.database.prepare(`
...
function queryTransportDescriptorPage(payload) {
  ...
  const sequenceQuery = catalogState.database.prepare(`
    SELECT id, state, item_count AS itemCount
    FROM playback_sequences WHERE id = ? AND state = 'active'
  `);
  const itemQuery = catalogState.database.prepare(`
  ...
  const mapCompositeOrdinal = catalogState.modules.transportShuffle.createTransportOrdinalMapper(descriptor, itemCount);
  const items = [];
  for (let ordinal = start; ordinal < end; ordinal += 1) {
    ...
    if (!mapSourceOrdinal) {
      mapSourceOrdinal = catalogState.modules.transportShuffle.createTransportOrdinalMapper(
        located.segment,
...
function ensureSystemPlaylist(kind, updatedAt) {
  ...
  catalogState.database.prepare('DELETE FROM playlist_items WHERE playlist_id = ?').run(definition.id);
  catalogState.database.prepare(`
    UPDATE playlists
...
function recordRecentlyPlayed(payload) {
  ...
  if (!catalogState.database.prepare('SELECT 1 FROM tracks WHERE track_uid = ?').get(trackUid)) {
    return { kind: 'noop' };
  }
  ...
  if (existing?.state === 'active') {
    ...
    const first = catalogState.database.prepare(`
  ...
  return commitMutation(['playlists', `playlist:${playlistId}`], 'record-recently-played', () => {
    ...
    const head = catalogState.database.prepare(`
    ...
    if (position <= 0) {
      ...
      `).all(playlistId);
      const updatePosition = catalogState.database.prepare(`
    ...
    const playlist = catalogState.database.prepare('SELECT version FROM playlists WHERE id = ?').get(playlistId);
...
function setTrackFavorite(payload) {
  ...
  if (favorite && !catalogState.database.prepare('SELECT 1 FROM tracks WHERE track_uid = ?').get(trackUid)) {
    return { kind: 'noop' };
  }
  if (existing?.state === 'building') {
    return { kind: 'busy', activeOperationId: existing.buildingOperationId ?? null };
  ...
  if (existing?.state === 'active') {
    const lease = findPlaylistLease(playlistId);
    if (lease) return { kind: 'busy', activeOperationId: lease };
    const item = catalogState.database.prepare(`
      SELECT item_key AS itemKey FROM playlist_items
...
function getSystemPlaylists(payload = {}) {
  ...
  return catalogState.database.prepare(`
...
function createPlaylistWithItems(payload) {
  ...
  if (catalogState.database.prepare('SELECT id FROM playlists WHERE id = ?').get(playlistId)) {
  ...
  try {
    for (let offset = 0; offset < items.length; offset += 1_000) {
      ...
      runDurableTransaction(() => {
        const playlist = catalogState.database.prepare(`
    ...
    return commitMutation(['playlists'], 'create-playlist-with-items', () => {
      const published = catalogState.database.prepare(`
        UPDATE playlists
...
function renamePlaylist(payload) {
  ...
  return commitMutation(['playlists'], 'rename-playlist', () => {
    const updated = catalogState.database.prepare(`
      UPDATE playlists
...
function removePlaylistItem(payload) {
  ...
  return commitMutation(['playlists'], 'remove-playlist-item', () => {
    const removed = catalogState.database.prepare(`
...
function reorderPlaylistItem(payload) {
  ...
  const visibleItem = itemKeyToFind => catalogState.database.prepare(`
    SELECT i.item_key AS itemKey, i.position
    FROM playlist_items i
    LEFT JOIN operation_jobs o ON o.operation_id = i.pending_operation_id
    WHERE i.item_key = ? AND i.playlist_id = ?
  ...
  return commitMutation(['playlists'], 'reorder-playlist-item', () => {
    ...
    if (upper - lower > 1 && Number.isSafeInteger(desiredPosition)) {
      ...
      catalogState.database.exec('DROP TABLE IF EXISTS temp.playlist_reorder_map');
      catalogState.database.exec(`
...
function duplicatePlaylist(payload) {
  ...
  const target = catalogState.database.prepare('SELECT id FROM playlists WHERE id = ?').get(targetPlaylistId);
...
function completeLocalPlaylistBuildOperation(operationId, result, finishedAt) {
  const completed = catalogState.database.prepare(`
    UPDATE operation_jobs
...
function assertPlaylistCopyState({
  ...
  const target = catalogState.database.prepare(`
    SELECT state, building_operation_id AS buildingOperationId
    FROM playlists WHERE id = ?
...
function prepareSequencePlaylistSave(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    if (playlist) {
      catalogState.database.prepare('UPDATE playlists SET updated_at = ? WHERE id = ?').run(createdAt, playlistId);
    } else {
      catalogState.database.prepare(`
        INSERT INTO playlists(
          id, name, sort_name, state, building_operation_id, version, created_at, updated_at
...
function appendSequencePlaylistPage(payload) {
  ...
  return runDurableTransaction(() => {
    const page = catalogState.database.prepare(`
...
function getAutomaticPlaylistImportState(payload) {
  ...
  const playlist = source ?? catalogState.database.prepare(`
...
function appendPlaylistImportRecords(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const playlist = catalogState.database.prepare(`
      ...
      FROM playlists WHERE id = ?
    `).get(playlistId);
    const automaticJob = catalogState.database.prepare(`
    ...
    const operationKind = catalogState.database.prepare(`
    ...
    let position = Number(catalogState.database.prepare(`
    ...
    let stagedCount = 0;
    const findAtPosition = catalogState.database.prepare(`
    ...
    `);
    const update = catalogState.database.prepare(`
...
function finalizePlaylistImportPage(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const remove = catalogState.database.prepare('DELETE FROM playlist_items WHERE item_key = ? AND pending_operation_id = ?');
    const finish = catalogState.database.prepare(`
      UPDATE playlist_items SET import_fields_json = NULL, import_has_path = NULL
      WHERE item_key = ? AND pending_operation_id = ?
    `);
    const resolve = catalogState.database.prepare(`
      UPDATE playlist_items
...
function resolveCuePlaylistTrack(unresolved) {
  ...
  if (folderId && entryKey) {
    const exact = catalogState.database.prepare(`
...
function tombstonePlaylist(payload) {
  ...
  const expectedVersion = requireNonNegativeInteger(payload.expectedVersion, 'expectedVersion');
  const updatedAt = requireNonNegativeInteger(payload.updatedAt, 'updatedAt');
  const playlist = catalogState.database.prepare('SELECT state, version FROM playlists WHERE id = ?').get(playlistId);
  if (!playlist || playlist.state === 'deleted') {
    throw createCatalogError('playlistNotFound', 'Playlist does not exist');
  }
  const lease = findPlaylistLease(playlistId);
  ...
  return commitMutation(['playlists'], 'tombstone-playlist', () => {
    catalogState.database.prepare('DELETE FROM automatic_playlist_sources WHERE playlist_id = ?').run(playlistId);
    catalogState.database.prepare(`
      UPDATE playlists
...
function cleanupPlaylistItems(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const pages = remaining === 0 ? [] : catalogState.database.prepare(`
      ...
      LIMIT ?
    `).all(remaining);
    const deletePage = catalogState.database.prepare('DELETE FROM sequence_save_pages WHERE rowid = ?');
...
function gcPlaylistItems(payload) {
  ...
  const limit = normalizeWriteLimit(payload.limit);
  return runDurableTransaction(() => {
    const failedItems = catalogState.database.prepare(`
      SELECT i.item_key AS itemKey
      FROM playlist_items i
      JOIN operation_jobs o ON o.operation_id = i.pending_operation_id
      WHERE o.terminal_kind IS NOT NULL
      ...
      LIMIT ?
    `).all(limit);
    const deleteItem = catalogState.database.prepare('DELETE FROM playlist_items WHERE item_key = ?');
    for (const item of failedItems) deleteItem.run(item.itemKey);

    let remaining = limit - failedItems.length;
    const failedPages = remaining === 0 ? [] : catalogState.database.prepare(`
      SELECT p.rowid
      FROM sequence_save_pages p
      JOIN operation_jobs o ON o.operation_id = p.operation_id
      WHERE o.terminal_kind IS NOT NULL
    ...
    const deletedItems = remaining === 0 ? [] : catalogState.database.prepare(`
      SELECT i.item_key AS itemKey
      FROM playlist_items i
    ...
    if (parentBudget > 0) {
      const playlists = catalogState.database.prepare(`
        SELECT p.id
        FROM playlists p
        WHERE p.state = 'deleted'
        ...
        LIMIT ?
      `).all(parentBudget);
      const deletePlaylist = catalogState.database.prepare('DELETE FROM playlists WHERE id = ?');
...
function normalizePlaylistItem(item) {
  ...
  const normalized = value => catalogState.modules.searchNormalizer.normalizeSearchText(
...
function getCachedArtwork(payload) {
  ...
  catalogState.database.prepare('UPDATE artwork_assets SET last_accessed_at = ? WHERE id = ?')
    .run(accessedAt, row.artworkId);
  return {
    kind: 'thumbnail',
...
function beginArtworkUtilitySession(payload) {
  ...
  catalogState.activeArtworkUtilitySession = requireString(payload.utilitySessionId, 'utilitySessionId', 512);
...
function claimArtworkSource(payload) {
  ...
  if (!catalogState.activeArtworkUtilitySession || claim.utilitySessionId !== catalogState.activeArtworkUtilitySession ||
...
function bindArtworkSourceDetails(payload) {
  ...
  if (row?.status !== 'extracting' || row.utilitySessionId !== catalogState.activeArtworkUtilitySession ||
...
function preflightArtworkBatch(payload) {
  ...
  const cacheBytes = Number(catalogState.database.prepare(`
...
function publishArtwork(payload) {
  ...
  const admitted = catalogState.database.prepare(`
  ...
  return commitMutation(
    ...
    () => {
      ...
      const now = Date.now();
      const existingAsset = catalogState.database.prepare(`
      ...
      if (!existingAsset) {
        ...
        catalogState.database.prepare('UPDATE artwork_assets SET last_accessed_at = ? WHERE id = ?').run(now, artworkId);
      }
      catalogState.database.prepare(`
      ...
      if (current.artworkId !== artworkId) {
        ...
        catalogState.database.prepare('UPDATE artwork_assets SET ref_count = ref_count + 1 WHERE id = ?').run(artworkId);
      }
      catalogState.database.prepare('UPDATE tracks SET artwork_id = ?, updated_at = ? WHERE track_uid = ?')
      ...
      catalogState.database.prepare('DELETE FROM artwork_claims WHERE claim_id = ?').run(claim.claimId);
...
function scheduleArtworkStagingGc(payload) {
  ...
  const result = catalogState.database.prepare('DELETE FROM artwork_claims WHERE claim_id = ? AND track_uid = ?')
...
function evictArtworkCache(payload) {
  ...
  let cacheBytes = Number(catalogState.database.prepare(`SELECT COALESCE(SUM(byte_length), 0) AS bytes FROM artwork_variants`).get().bytes);
  let evictedBytes = 0;
  const candidates = catalogState.database.prepare(`
  ...
  for (const candidate of candidates) {
    ...
    catalogState.database.prepare('DELETE FROM artwork_variants WHERE artwork_id = ? AND variant = ?')
...
function artworkPreclaimMatchesCurrentTrack(claim) {
  ...
  const matchesTrack = Boolean(track) && claim.utilitySessionId === catalogState.activeArtworkUtilitySession &&
...
function artworkClaimIsCurrent(claim, { requireAdmission = false } = {}) {
  ...
  return row?.status === 'extracting' && (!requireAdmission || row.admittedAt != null) &&
    row.utilitySessionId === catalogState.activeArtworkUtilitySession &&
...
function commitScanSeenBatch(payload) {
  ...
  return runDurableTransaction(() => {
    ...
    const upsert = catalogState.database.prepare(`
    ...
    `);
    const upsertLogical = catalogState.database.prepare(`
    ...
    let logicalSequence = Number(catalogState.database.prepare(`
...
function clearCueDirectoryStageRows(identity, directoryPath) {
  for (const table of [
    ...
    catalogState.database.prepare(`DELETE FROM ${table} WHERE scan_id = ? AND folder_id = ? AND directory_path = ?`)
...
function clearCueScanStageRows(identity) {
  for (const table of [
    ...
    catalogState.database.prepare(`DELETE FROM ${table} WHERE scan_id = ? AND folder_id = ?`)
...
function getCueDirectoryStageFile({ identity, directoryPath, payload }) {
  ...
  const file = catalogState.database.prepare(`
    SELECT relative_path AS relativePath, canonical_path AS path, entry_sequence AS sequence,
      entry_kind AS kind, file_identity AS fileIdentity, size, mtime_ms AS mtimeMs,
      metadata_status AS metadataStatus
    FROM scan_cue_stage_files
...
function resolveCueDirectoryStageReferences({ identity, directoryPath, payload }) {
  ...
  const selectExact = catalogState.database.prepare(`
    ...
    ORDER BY entry_sequence
  `);
  const selectFolded = catalogState.database.prepare(`
    SELECT relative_path AS relativePath FROM scan_cue_stage_files
    WHERE scan_id = ? AND folder_id = ? AND directory_path = ?
...
function stageCueDirectorySheet({ identity, directoryPath, payload }) {
  ...
  return runDurableTransaction(() => {
    ...
    const insertTrack = catalogState.database.prepare(`
...
function updateCueDirectoryStageSource({ identity, directoryPath, payload }) {
  ...
  return runDurableTransaction(() => {
    const result = catalogState.database.prepare(`
...
function getCueDirectoryStageSourceMetadata({ identity, directoryPath, payload }) {
  ...
  const select = catalogState.database.prepare(`
...
function validateCueDirectoryStageSheet({ identity, directoryPath, payload }) {
  ...
  return runDurableTransaction(() => {
    ...
    if (payload.valid === true) {
      ...
      const missing = catalogState.database.prepare(`
...
function acceptCueDirectoryStageSheet({ identity, directoryPath, payload }) {
  const cueRelativePath = normalizeRelativePath(requireString(payload.cueRelativePath, 'cueRelativePath', 32768));
  return runDurableTransaction(() => {
    const sources = catalogState.database.prepare(`
    ...
    const collision = catalogState.database.prepare(`
...
function listCueDirectoryStageLogical({ identity, directoryPath, payload }) {
  ...
  const sheet = catalogState.database.prepare(`
  ...
  const tracks = catalogState.database.prepare(`
...
function enqueueScanSweep(payload) {
  ...
  catalogState.database.prepare(`UPDATE scan_run_folders SET status = 'sweeping', updated_at = ? WHERE scan_id = ? AND folder_id = ? AND generation = ?`)
...
function deleteScanSweepRows(identity, rows) {
  return commitMutation(
    ...
    'scan-sweep',
    () => {
      const insertDeletionJob = catalogState.database.prepare(`
      ...
      for (const row of rows) {
        ...
        catalogState.database.prepare('DELETE FROM tracks WHERE track_uid = ?').run(row.trackUid);
...
function repairPlaylistItemsForTrack(track, limit, deletionJobId) {
  ...
  for (const item of page) {
    ...
    const changed = repair.run(
      unresolved,
      catalogState.modules.searchNormalizer.normalizeSearchText(track.fileName),
      catalogState.modules.searchNormalizer.normalizeSearchText(track.title),
      catalogState.modules.searchNormalizer.normalizeSearchText(track.artist),
...
function setScanTerminal(payload, status) {
  ...
  const result = runDurableTransaction(() => {
    ...
    catalogState.database.prepare(`UPDATE folders SET last_scan_at = ? WHERE id = ?`).run(now, identity.folderId);
    if (status === 'paused') {
      catalogState.database.prepare(`
    ...
    catalogState.database.prepare(`UPDATE scan_runs SET status = ?, finished_at = ?, stop_reason = ? WHERE id = ?`)
  ...
  return commitMutation(changedScopes, 'metadata-claim', () => {
    ...
    if (existing) {
      ...
      if (artworkSourceChanged) {
        catalogState.database.prepare('UPDATE tracks SET artwork_id = NULL WHERE track_uid = ?').run(trackUid);
        catalogState.database.prepare('DELETE FROM track_artwork_sources WHERE track_uid = ?').run(trackUid);
        catalogState.database.prepare(`
          UPDATE artwork_assets SET ref_count = CASE WHEN ref_count > 0 THEN ref_count - 1 ELSE 0 END
          WHERE id = ?
      ...
      const normalize = catalogState.modules.searchNormalizer.normalizeSearchText;
      const searchText = catalogState.modules.searchNormalizer.createCompactSearchText([
...
function replaceTrackEntityMemberships(track, metadata, { recomputeAggregates = true } = {}) {
  const prior = {
    album: catalogState.database.prepare('SELECT album_key AS key FROM track_albums WHERE track_uid = ?').get(track.trackUid)?.key ?? null,
    artists: catalogState.database.prepare('SELECT artist_key AS key FROM track_artists WHERE track_uid = ?').all(track.trackUid).map(row => row.key),
    genres: catalogState.database.prepare('SELECT genre_key AS key FROM track_genres WHERE track_uid = ?').all(track.trackUid).map(row => row.key),
    subfolder: catalogState.database.prepare('SELECT subfolder_key AS key FROM track_subfolders WHERE track_uid = ?').get(track.trackUid)?.key ?? null
  };
  const next = derivedTrackEntities(track, metadata);
  catalogState.database.prepare('DELETE FROM track_albums WHERE track_uid = ?').run(track.trackUid);
  catalogState.database.prepare('DELETE FROM track_artists WHERE track_uid = ?').run(track.trackUid);
  catalogState.database.prepare('DELETE FROM track_genres WHERE track_uid = ?').run(track.trackUid);
  catalogState.database.prepare('DELETE FROM track_subfolders WHERE track_uid = ?').run(track.trackUid);
  ...
  const upsertArtist = catalogState.database.prepare(`
  ...
  catalogState.database.prepare('INSERT INTO track_albums(track_uid, album_key) VALUES (?, ?)').run(track.trackUid, next.album.key);
  const insertTrackArtist = catalogState.database.prepare('INSERT INTO track_artists(track_uid, artist_key, role) VALUES (?, ?, ?)');
  for (const artist of next.artists) insertTrackArtist.run(track.trackUid, artist.key, 'album-artist');
  catalogState.database.prepare('INSERT INTO track_genres(track_uid, genre_key) VALUES (?, ?)').run(track.trackUid, next.genre.key);
  if (next.subfolder) {
    catalogState.database.prepare('INSERT INTO track_subfolders(track_uid, subfolder_key) VALUES (?, ?)').run(track.trackUid, next.subfolder.key);
  }
  catalogState.database.prepare(`
...
function recomputeAggregateRows(entityTable, membershipTable, keyColumn, keys) {
  const read = catalogState.database.prepare(`
  ...
  const update = catalogState.database.prepare(`
    ...
    WHERE ${keyColumn} = ?
  `);
  const remove = catalogState.database.prepare(`DELETE FROM ${entityTable} WHERE ${keyColumn} = ?`);
...
function recomputeArtworkAggregateRowsForTrack(trackUid) {
  const album = catalogState.database.prepare('SELECT album_key AS key FROM track_albums WHERE track_uid = ?').get(trackUid)?.key;
  const artists = catalogState.database.prepare('SELECT artist_key AS key FROM track_artists WHERE track_uid = ?').all(trackUid);
  const genres = catalogState.database.prepare('SELECT genre_key AS key FROM track_genres WHERE track_uid = ?').all(trackUid);
  const subfolder = catalogState.database.prepare('SELECT subfolder_key AS key FROM track_subfolders WHERE track_uid = ?').get(trackUid)?.key;
...
function removeTrackEntityMemberships(trackUid) {
  const prior = {
    album: catalogState.database.prepare('SELECT album_key AS key FROM track_albums WHERE track_uid = ?').get(trackUid)?.key ?? null,
    artists: catalogState.database.prepare('SELECT artist_key AS key FROM track_artists WHERE track_uid = ?').all(trackUid).map(row => row.key),
    genres: catalogState.database.prepare('SELECT genre_key AS key FROM track_genres WHERE track_uid = ?').all(trackUid).map(row => row.key),
    subfolder: catalogState.database.prepare('SELECT subfolder_key AS key FROM track_subfolders WHERE track_uid = ?').get(trackUid)?.key ?? null
  };
  catalogState.database.prepare('DELETE FROM track_albums WHERE track_uid = ?').run(trackUid);
  catalogState.database.prepare('DELETE FROM track_artists WHERE track_uid = ?').run(trackUid);
  catalogState.database.prepare('DELETE FROM track_genres WHERE track_uid = ?').run(trackUid);
  catalogState.database.prepare('DELETE FROM track_subfolders WHERE track_uid = ?').run(trackUid);
...
function removeTrackArtworkReferences(trackUid) {
  const artworkId = catalogState.database.prepare('SELECT artwork_id AS artworkId FROM tracks WHERE track_uid = ?')
    .get(trackUid)?.artworkId ?? null;
  catalogState.database.prepare('DELETE FROM artwork_claims WHERE track_uid = ?').run(trackUid);
  catalogState.database.prepare('DELETE FROM track_artwork_sources WHERE track_uid = ?').run(trackUid);
  if (artworkId != null) {
    catalogState.database.prepare(`
      UPDATE artwork_assets SET ref_count = CASE WHEN ref_count > 0 THEN ref_count - 1 ELSE 0 END
...
function completeMetadataParseSuccess(payload) {
  ...
  const result = commitMutation(['tracks', 'albums', 'artists', 'genres', 'subfolders'], 'metadata-success', () => {
    const track = catalogState.database.prepare(`
    ...
    catalogState.database.prepare('DELETE FROM metadata_claims WHERE folder_id = ? AND logical_storage_id = ?')
      .run(claim.folderId, claim.logicalStorageId);
    return { committed: true };
  }, { deferInvalidationKey: entityAggregationScanKey(claim) });
  if (result.committed && pendingAggregateScanKey) {
    if (catalogState.activeMutationBatch) catalogState.activeMutationBatch.pendingAggregationScans.add(pendingAggregateScanKey);
...
function runMetadataMutationBatch(scanKey, reason, callback) {
  ...
  const result = commitMutation(changedScopes, reason, () => {
    if (catalogState.activeMutationBatch) throw createCatalogError('invalidMetadataBatch', 'Metadata mutation batches cannot be nested');
    catalogState.activeMutationBatch = { changedScopes, pendingAggregationScans };
    try {
      return callback();
    } finally {
      catalogState.activeMutationBatch = null;
    }
...
function requeueLatestMetadata(payload) {
  ...
  const currentClaim = catalogState.database.prepare(`
...
function recoverInterruptedMetadataClaims(payload) {
  ...
  return commitMutation(['tracks'], 'metadata-recovery', () => {
    ...
    catalogState.database.prepare('DELETE FROM metadata_claims').run();
...
function runFolderDeletionChunk(folderId, lifecycleVersion) {
  ...
  const job = catalogState.database.prepare(`
...
function runFolderDeletionChunkInTransaction(folderId, lifecycleVersion, deletionJobId) {
  ...
  const nextTrack = catalogState.database.prepare(`
    ...
    `);
  const deleteRepairItems = catalogState.database.prepare(`
    DELETE FROM deletion_repair_items WHERE job_id = ? AND original_track_uid = ?
  `);
  const updateJob = catalogState.database.prepare(`
  ...
  while (deleted < FOLDER_DELETION_TRACKS_PER_CHUNK) {
    ...
    if (!row) {
      catalogState.database.prepare(`UPDATE deletion_jobs SET state = 'completed', updated_at = ? WHERE job_id = ?`)
...
function ensurePendingEntityAggregationJob(claim) {
  ...
  const identity = catalogState.database.prepare(`
    SELECT scan_id AS scanId, folder_id AS folderId, generation,
...
function runPlaylistResolutionChunk(jobId, folderId, lifecycleVersion, cursorKey) {
  ...
  const apply = () => {
    ...
    const now = Date.now();
    const updatePlaylist = catalogState.database.prepare(`
      UPDATE playlists SET version = version + 1, updated_at = ?
      WHERE id = ? AND state = 'active'
    `);
...
function scheduleDeletionMaintenance() {
  if (catalogState.closed || catalogState.deletionMaintenanceScheduled) return;
  catalogState.deletionMaintenanceScheduled = true;
  setTimeout(() => {
    catalogState.deletionMaintenanceScheduled = false;
    if (catalogState.closed) return;
    try {
...
function runDeletionMaintenanceTurn() {
  ...
  const active = catalogState.database.prepare(`
  ...
  const orphan = catalogState.database.prepare(`
...
function repairBlockedDeletionItems(limit) {
  ...
  return commitMutation(['playlists'], 'repair-interrupted-deletion', () => {
    const trackExists = catalogState.database.prepare('SELECT 1 AS present FROM tracks WHERE track_uid = ?');
    const rebind = catalogState.database.prepare(`
      UPDATE playlist_items
      SET track_uid = ?, unresolved_json = NULL, unresolved_basename = NULL,
        unresolved_title = NULL, unresolved_artist = NULL, unresolved_duration_bucket = NULL
      WHERE item_key = ? AND track_uid IS NULL
    `);
    const remove = catalogState.database.prepare('DELETE FROM deletion_repair_items WHERE job_id = ? AND item_key = ?');
    const affectedPlaylists = new Set();
    for (const row of rows) {
...
function closeDatabase() {
  if (!catalogState.database) return;
  try {
    catalogState.database.close();
  } finally {
    catalogState.database = null;
  }
}
...
function flushPendingScanInvalidation(scanKey) {
  ...
  const invalidation = {
    ...
    changedScopes,
    scopeVersions: Object.fromEntries(changedScopes.map(scope => [scope, catalogState.scopeVersions[scope]])),
...
function validateCatalogVersion(value) {
  ...
  if (value !== catalogState.catalogVersion) throw createCatalogError('STALE_CURSOR', 'Catalog version is stale');
}
...
function createSortKey(value) {
  return Buffer.from(catalogState.modules.orderContract.encodeCanonicalSortKey(value), 'hex');
}
...
export {
  ...
  MAX_TRACKS_PER_UPSERT_STATEMENT,
  MAX_QUERY_CHARACTERS,
  MAX_QUERY_TOKENS,
  MAX_QUERY_TOKEN_CHARACTERS,
  FOLDER_DELETION_TRACKS_PER_CHUNK,
  DELETION_MAINTENANCE_DELAY_MS,
  MAX_TRACK_TEXT_CHARACTERS,
  MAX_DIRECTORY_PATH_CHARACTERS,
  MAX_FOLDER_DIRECTORY_KEY_CHARACTERS,
  DIRECTORY_RESPONSE_BYTE_BUDGET,
  MAX_ARTWORK_RAW_BYTES,
  MAX_ARTWORK_THUMBNAIL_BYTES,
  ARTWORK_STORAGE_SAFETY_MIN_BYTES,
  ARTWORK_STORAGE_SAFETY_MAX_BYTES,
  PLAYLIST_RESOLUTION_CANDIDATE_LIMIT,
  PLAYLIST_RECONCILIATION_BATCH_SIZE,
  SYSTEM_PLAYLIST_IDS,
  SYSTEM_PLAYLIST_DEFINITIONS,
  RECENTLY_PLAYED_LIMIT,
  FAVORITE_TRACK_UID_LIMIT,
  ENTITY_AGGREGATE_PHASES,
  RECENT_TRACK_LIMIT,
  ...
  normalizeContextEndpoint,
  normalizeScope,
  normalizeEntityType,
  queryTracks,
  queryEntities,
  assertContextQueryMatches,
  validateContextCatalogVersion,
  readContextPage,
  readContextPageAtOrdinal,
  createOrdinalPagePlan,
  resolveEntityAnchor,
  resolveNamedEntityContextOrdinal,
  readPlaylistContextPage,
  createPlaylistOrder,
  createPlaylistContextFilter,
  createPlaylistKeysetSql,
  createPlaylistTrackPageSelection,
  readEntityContextPage,
  readEntityContextPageAtOrdinal,
  executeEntityContextPage,
  createEntityOrder,
  entityOrderColumn,
  createEntityContextFilter,
  countEntityContextRows,
  createEntityPageSelection,
  createEntityOrderBySql,
  createEntityKeysetSql,
  createLexicographicPredicate,
  createEntityNullRankExpression,
  entityFieldExpression,
  entityContextHasRows,
  stripEntityOrderFields,
  encodeEntityBoundaryCursor,
  ...
  boundedTransportDescriptor,
  normalizeDurableTransportDescriptor,
  transportDescriptorItemCount,
  locateTransportSegment,
  ...
  normalizeFavoriteTrackUidCursor,
  getSystemPlaylists,
  createPlaylist,
  ...
  reorderPlaylistItem,
  duplicatePlaylist,
  beginLocalPlaylistBuild,
  completeLocalPlaylistBuildOperation,
  getActivePlaylistForLocalMutation,
  updatePlaylistVersionForLocalMutation,
  readVisiblePlaylistCopyPage,
  appendPlaylistCopyPage,
  assertPlaylistCopyState,
  failPlaylistCopy,
  ...
  playlistImportPreviewLabel,
  resolveCuePlaylistTrack,
  playlistPathSuffixScore,
  normalizePlaylistImportFields,
  playlistImportUnresolved,
  publishPlaylist,
  queryPlaylistItems,
  tombstonePlaylist,
  ...
  validateBoundedResultObject,
  assertPlaylistOperation,
  assertOperationCanCommit,
  findPlaylistLease,
  ...
  evictArtworkCache,
  normalizeArtworkSourceClaim,
  normalizeArtworkThumbnail,
  artworkSourceSignaturesEqual,
  getArtworkTrack,
  artworkPreclaimMatchesCurrentTrack,
  normalizeArtworkFileStat,
  artworkClaimIsCurrent,
  canonicalArtworkClaimJson,
  normalizeExternalArtworkStat,
  normalizeArtworkCachePolicy,
  artworkStorageAdmission,
  getScanFolderTrackCount,
  completePendingScanSweepRecovery,
  preflightScanBatch,
  commitScanSeenBatch,
  cueDirectoryStage,
  resetCueDirectoryStage,
  clearCueDirectoryStageRows,
  clearCueScanStageRows,
  normalizeCueStageDirectory,
  cueStageParentPath,
  cueStageFileName,
  appendCueDirectoryStageEntries,
  listCueDirectoryStageFiles,
  getCueDirectoryStageFile,
  updateCueDirectoryStageObservations,
  resolveCueDirectoryStageReferences,
  stageCueDirectorySheet,
  listCueDirectoryStageSources,
  updateCueDirectoryStageSource,
  listCueDirectoryStageSheets,
  getCueDirectoryStageSourceMetadata,
  validateCueDirectoryStageSheet,
  acceptCueDirectoryStageSheet,
  listCueDirectoryStageLogical,
  ...
  runScanSweep,
  deleteScanSweepRows,
  repairPlaylistItemsForTrack,
  createSourceRemovedPlaylistItem,
  completeTrackDeletionRepair,
  completeScanFolder,
  ...
  claimMetadataParse,
  aggregateIdentity,
  derivedTrackEntities,
  replaceTrackEntityMemberships,
  recomputeAggregateRows,
  recomputeArtworkAggregateRowsForTrack,
  ...
  requireMetadataBatchScanKey,
  runMetadataMutationBatch,
  requeueLatestMetadata,
  recoverInterruptedMetadataClaims,
  runFolderDeletionChunk,
  runFolderDeletionChunkInTransaction,
  ensureFolderDeletionJob,
  folderDeletionJobId,
  ensurePendingEntityAggregationJob,
  entityAggregationScanKey,
  activateEntityAggregationJob,
  entityAggregationJobId,
  recomputeAllAggregateRows,
  runEntityAggregationPhase,
  ensurePlaylistResolutionJob,
  playlistResolutionJobId,
  listPlaylistResolutionCandidates,
  runPlaylistResolutionChunk,
  scheduleDeletionMaintenance,
  runDeletionMaintenanceTurn,
  hasDeletionMaintenanceWork,
  repairBlockedDeletionItems,
  bumpActivePlaylistVersions,
  validateBoundedStringList,
  requireActiveScanFolder,
  requireScanIdentity,
  requireScanState,
  insertScanError,
  normalizeScanSignature,
  signaturesEqual,
  createPlainScanLogicalCandidate,
  normalizeScanLogicalCandidate,
  normalizeMetadataSourceDescriptor,
  validateMetadataClaim,
  currentMetadataClaim,
  normalizeParsedMetadata,
  normalizeAlbumArtists,
  closeDatabase,
  flushPendingScanInvalidation,
  pickCounts,
  ...
  withPhysicalSourceKey,
  createSortKey,
  normalizeExtensionJson,
  optionalString,
